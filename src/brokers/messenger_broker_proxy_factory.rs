...
use super::messenger_broker_proxy::{MessengerBrokerProxy, MessengerBrokerProxyCoreFactory};
...
impl BrokerProxyFactory for MessengerBrokerProxyFactory {
    ...
    fn create_broker_proxy(&self, manifest: Value) -> JuizResult<Arc<Mutex<dyn BrokerProxy>>>{
        ...
        let name = obj_get_str(&manifest, "name").context("LocalBrokerProxyFactory::create_broker_proxy")?;
        let class_name = "";
        let type_name = "";
        let object_name = "";
