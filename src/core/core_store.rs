use std::{cell::RefCell, collections::{hash_map::Values, HashMap}, rc::Rc, sync::{Arc, Mutex, RwLock}};
...
impl<T, TF> BufferStoreWorker<T, TF> where T: JuizObject + ?Sized, TF: JuizObject + ?Sized {

    pub fn new(name: &str) -> Box<BufferStoreWorker<T, TF>> {
