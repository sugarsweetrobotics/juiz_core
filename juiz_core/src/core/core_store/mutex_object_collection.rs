...
pub struct MutexObjectCollection<T, TF> where T: JuizObject + ?Sized, TF: JuizObject + ?Sized {
    name: String,
...
impl<T, TF> MutexObjectCollection<T, TF> where T: JuizObject + ?Sized, TF: JuizObject + ?Sized {

    pub fn new(name: &str) -> Box<MutexObjectCollection<T, TF>> {
        Box::new(MutexObjectCollection { name: name.to_string(), 
