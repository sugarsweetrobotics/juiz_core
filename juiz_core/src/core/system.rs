...
use crate::brokers::{
    broker_factories_wrapper::BrokerFactoriesWrapper, broker_proxy::SystemBrokerProxy,
};
...
fn check_system_manifest(manifest: Value) -> JuizResult<Value> {
    if !manifest.is_object() {
        return Err(
            anyhow::Error::from(JuizError::ValueIsNotObjectError { value: manifest })
                .context("check_system_manifest failed."),
        );
    }
    return Ok(manifest);
}
...
impl System {
    ...
    pub fn setup(mut self) -> JuizResult<Self> {
        ...
        let _ = when_contains_do_mut(&manifest_copied, "plugins", |v| {
            system_builder::setup_plugins(&mut self, v, &option)
                .context("system_builder::setup_plugins in System::setup() failed")
        })?;

        system_builder::setup_objects(&mut self, &manifest_copied)
            .context("setup_objects() failed in setup in system.rs")?;
                    ...
                    .map(|(type_name, broker)| {
                        ...
                        let p = match broker.lock_mut() {
                            Ok(mut b) => {
                                ...
                                Ok((b.profile(), broker.clone()))
                            }
                            Err(e) => {
                                log::error!("Broker({type_name:}) lock failed. Error({e:?})");
                                Err(anyhow!(JuizError::ObjectLockError {
                                    target: format!("Broker({type_name})")
                                }))
                            }
                        }?;
                        Ok(p)
                    })
                    .collect::<JuizResult<Vec<(BrokerProfile, BrokerPtr)>>>()?;
                profs.into_iter().for_each(|(v, _b)| {
                    let type_name = v.type_name.clone();
                    let _ = self
                        .core_broker()
                        .lock_mut()
                        .unwrap()
                        .worker_mut()
                        .store_mut()
                        .register_broker_manifest(v)
                        .or_else(|e| {
                            log::error!(
                                "【エラー】Store::register_broker({type_name})失敗。エラー({e:?})"
                            );
                            Err(e)
                        });
                });
            ...
            Ok(mut cb) => {
                ...
                if opt_value.contains_key("http_broker") {
                    opt_value
                        ...
                        .as_object_mut()
                        .unwrap()
                        .insert("start".to_owned(), jvalue!(flag_start));
                } else {
    ...
    pub fn run(&mut self) -> JuizResult<()> {
        ...
        self.wait_for_singal()
            .context("System::wait_for_signal() in System::run() failed.")?;
        self.stop()?;
    ...
    pub fn run_and_do_once(
        ...
        log::info!(
            "【run_and_do_once】Juizシステム({})スタートしました。",
            self.store.uuid()?
        );
    ...
    pub fn register_broker_factories_wrapper(
        &mut self,
        bf: Arc<Mutex<BrokerFactoriesWrapper>>,
    ) -> JuizResult<Arc<Mutex<BrokerFactoriesWrapper>>> {
        ...
        if self.store.lock()?.broker_factories.contains_key(&type_name) {
            ...
            return Err(anyhow::Error::from(
                JuizError::BrokerFactoryOfSameTypeNameAlreadyExistsError {
                    type_name: type_name,
                },
            ));
        }
        self.store
            .lock_mut()?
            .broker_factories
            .insert(type_name.clone(), Arc::clone(&bf));
        self.core_broker()
            .lock_mut()?
            .worker_mut()
            .store_mut()
            .register_broker_factory_manifest(
                type_name.as_str(),
                juiz_lock(&bf)?.profile_full()?.try_into()?,
            )?;
        self.core_broker()
            .lock_mut()?
            .worker_mut()
            .store_mut()
            .broker_proxies
            .register_factory(juiz_lock(&bf)?.broker_proxy_factory.clone())?;
        log::trace!(
            "System::register_broker_factories_wrapper(BrokerFactory(type_name={:?})) exit",
            type_name
        );
        Ok(bf)
    }

    fn broker_factories_wrapper(
        &self,
        type_name: &str,
    ) -> JuizResult<Arc<Mutex<BrokerFactoriesWrapper>>> {
        match self.store.lock()?.broker_factories.get(type_name) {
            None => Err(anyhow::Error::from(
                JuizError::BrokerFactoryCanNotFoundError {
    ...
    pub fn create_broker(&mut self, manifest: &Value) -> JuizResult<BrokerPtr> {
        ...
        let b = juiz_lock(&bf)?
            .create_broker(&manifest)
            .context("BrokerFactoriesWrapper.create_broker() failed in System::create_broker()")?;
    ...
    pub fn create_broker_proxy(
        ...
        let bp = self
            .core_broker()
            .lock()?
            .create_broker_proxy(manifest.clone())?;
        self.register_broker_proxy(bp)
    }

    pub(crate) fn register_broker_proxy(
        &mut self,
        broker_proxy: Arc<Mutex<dyn BrokerProxy>>,
    ) -> JuizResult<Arc<Mutex<dyn BrokerProxy>>> {
        let type_name = juiz_lock(&broker_proxy)
            .context("Locking broker to get type_name failed.")?
            .type_name()
            .to_string();
    ...
    pub fn container_process_list(&self, recursive: bool) -> JuizResult<Vec<ProcessIdentifier>> {
        ...
        let mut local_processes = self
            .core_broker()
            .lock()?
            .worker()
            .store()
            .container_processes_id();
        for (_, proxy) in self
            .core_broker()
            .lock()?
            .worker()
            ...
            .iter()
        {
            for v in juiz_lock(proxy)?
                .container_process_list(recursive, None)?
                .iter()
            {
                local_processes.push(v.clone());
            }
    ...
    match home_dir() {
        Some(homepath) => {
            ...
            if juiz_default_conf_filepath.exists() {
                ...
                let system_manifest = yaml_conf_load_with(
                    juiz_default_conf_filepath.to_str().unwrap().to_owned(),
                    param_map(juiz_homepath),
                )?;
