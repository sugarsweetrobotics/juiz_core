...
機能モジュールのユーザーのシステム開発が進み、アプリケーションのサービスドメインに適用した場合、これはドメインによって多種多様であり、これを想定することは不可能だと筆者は考える。
一方で、現在普及しているROSやOpenRTM-aistは、機能モジュールの設計方法として冗長かつ柔軟性に欠けた選択がなされており、これらのモデルの延長線上で上記の問題を克服するのは難しいと考えている。
...
このような状態の集中管理はデバッグのしやすさや、コンポーネントと外部とのインターフェースの設計困難問題に対して一定の評価ができるが、そもそも値の入出力に対して方法論が複数ある中で、新たな方法論を導入するのはミドルウェアの無意味な肥大化に繋がるので、新しい哲学を持ったミドルウェアの設計を提案することにする。

繰り返すが、ソフトウェア規模が大きくなるとコンポーネントが内包する状態を隠蔽しすぎることの弊害が指摘される一方で、ROSに見られるコンポーネント型かつ比較的厳格な直積型データ型を利用するアーキテクチャでは、心筋梗塞を起こしやすい。
...
内部の状態を外部と入出力する場合に、設計者はこれらの中から一つ、ないしは複数を選択する必要がある点で困難である。
...
これを後述するコンテナプロセスと区別するために「純粋プロセス」と呼ぶことにする。
...
プロセスは実体化すると各引数にと出力にバッファ (outlet, inlet) を持つが、connectでは、プロセスの出力 (outlet) を別のプロセスの引数の一つ (inlet) に繋ぐことができる。
connectionに繋がったプロセスのうち、出力する側をsourceとよび、入力を受ける側をdestinationと呼ぶ。
connectionにはタイプがある。
あるプロセスがexecuteされると、そのinletのうち、pull型をconnectionを持つものは、そのconnectionのsourceに対してexecuteを要求し、出力を受け取る。
...
プロセスがexecuteされたときにinletにconnectionがない場合や、connectionがすべてpull型出ない場合はキャッシュの値を引数に束縛する。
